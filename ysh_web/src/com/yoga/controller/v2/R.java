...
public class R {
	...
	protected String create_nonce_str() {
		return PayCommonUtil.CreateNoncestr();
	}
	 ...
	 * @return
	 */
	public int sendBuyCoureseTemplate( String openId, Weixin_blog_saler_order course){
		String url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token="+weixin.getToken()[0];
		WxTemplate temp = new WxTemplate();
		String money = course.getMoney().toString();
		if (money.indexOf(".")>-1){
			money = money.substring(0,money.indexOf("."));
		}
		temp.setUrl( DOMAIN+"/pay/live/detail/"+money+"_"+course.getProductid()+".shtml");
		temp.setTouser( openId );
		temp.setTopcolor("#000000");
		temp.setTemplate_id( Config.getString("weixin.buycourse.templateid") );
		...
		TemplateData keyword2 = new TemplateData();
		keyword2.setColor("#000000");
		keyword2.setValue( course.getProductname() );
		m.put("keyword2", keyword2);
