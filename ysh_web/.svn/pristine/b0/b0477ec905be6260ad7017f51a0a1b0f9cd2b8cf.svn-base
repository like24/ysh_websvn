 ...
 <body>
	<div class="wrapper"> 
		<section class="mainBox" id="mainBox" >
              <div >
				  ...
				  <h2>共收藏${ contentlist.totalCount }个线下培训
				</div>
                  <ul id="list" class="list" >
                    <c:forEach items="${contentlist.results}" var="course" varStatus="status">
	                      <li data-url="${ctx}/w/faq/courseline.html?p=${course.id}" >
	                          ...
	                          <dl>
	                              <dt>
	                                 <a href="${ctx}/w/faq/person.html?p=${course.luserid}">
	                                    <c:choose>
											<c:when test="${ fn:indexOf(course.lheader,'http') > -1 }"><img src="${course.lheader}"  ></c:when>
											<c:otherwise><img src="${ctx}/${course.lheader}"  ></c:otherwise>    
										 </c:choose>  
	                                  </a>
	                              </dt>
	                              <dd>
	                                  <p><span>${course.lusername }</span><b>|</b><span class="summary" >${course.luserpost }</span></p>
	                              </dd>
	                          </dl>
	                      </li>
                      </c:forEach>
	...
	<script>
		$(".nav li.taplink").on("tap",function(){
			window.location.href="${ctx}/w/my/minecol.html";
		})
