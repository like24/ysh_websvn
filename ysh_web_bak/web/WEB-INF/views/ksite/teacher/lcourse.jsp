    ...
    <div class="wrapper">
        <section class="mainBox" id="mainBox">
         ...
         <nav class="nav">
            ...
            <c:if test="${u.isacceptask != 1}">
           <div class="content">
              <h3>共 ${resultlist.totalCount} 个线下培训</h3>
              <ol id="list">
                 <c:if test="${ resultlist.totalCount > 0 }">
	                <c:forEach items="${resultlist.results}" var="course" varStatus="status">
		                <li class="taplist"  onclick="sel(this)" data-url="${ctx}/w/faq/courseline.html?p=${course.id}">
		                  <h1>${course.cname }</h1>
		                   <p>${course.remark }</p>
	    ...
	    function loadStore(){
			if( pageIndex > -1 ){
				$.ajax({
	 				type:"post",
	 				dataType:"json",
	 				url:  "${ctx}/w/faq/person/json.html?p=${p}&t=2&pageIndex="+(pageIndex+1),
	 				error:function(XMLHttpRequest,textStatus,error){},
	 				success:function(msg){
	 					if( msg.result != ''){
