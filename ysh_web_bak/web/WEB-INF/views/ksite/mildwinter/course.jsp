...
/* 	    if(isteacher == "1"){
	...
	function playMp3( jdom ){ 
		...
		function playNextMp3(btn){
			...
			$("#audio111").get(0).pause();
			btn.next("b").remove();
			btnfind("img").css({"width":"4.56rem"}).attr("src", "http://wimg.keepyoga.com/loding_002.gif");
			$playjdom = btn;
			var mp3source = btn.attr("data-url");
			playnum = btn.attr("number");
			$("#audio111").get(0).pause();
			$("#audio111").get(0).src= base + mp3source;
			$("#audio111").get(0).volume=1;
			$("#audio111").get(0).play();
		...
		function playstart(){
			...
			playtimer = setTimeout(function(){
				...
				if(playnum < btnnum){
					playnum = parseInt(playnum) + 1;
					var $b = $("button[number='"+playnum+"']");
					if($b.next().is("b")){
						playNextMp3($b);
					}
				}
